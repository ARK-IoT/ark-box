...
CONFIG_PATH="/opt/arkbox/conf/hostapd.conf"
ARKBOX_CONFIG_PATH="/opt/arkbox/conf/arkbox.conf"
...
arkbox_interface=$( grep -e '^INTERFACE' "${ARKBOX_CONFIG_PATH}" | \
                sed -e 's|INTERFACE=||' -e 's|"||g' )
dnsmasq_interface=$( grep -e '^DNSMASQ_INTERFACE' "${ARKBOX_CONFIG_PATH}" | \
                sed -e 's|DNSMASQ_INTERFACE=||' -e 's|"||g' )

sed -i -e "s|interface=$hostap_interface|interface=$WIFI_DEVICE|" "${CONFIG_PATH}"

#Only change arkbox interface if it is a wifi interface
if echo "$arkbox_interface" | grep -q "wlan" ; then
    sed -i -e "s|INTERFACE=\"$arkbox_interface\"|INTERFACE=\"$WIFI_DEVICE\"|" \
         "${ARKBOX_CONFIG_PATH}"
fi
...
if [ -f /boot/wpa_supplicant.conf ]; then
  /opt/arkbox/rpi/bin/run_client.sh
  exit 1
  # Exit =! 0 will result in not starting arkbox service
fi
